...
export const EditOrderModal: React.FC<EditOrderModalProps> = ({ isOpen, onClose, order, onSave }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Check if the typed client name exists in the local list
            const matchedClient = clients.find(c => c.name.toLowerCase().trim() === formData.clientName.toLowerCase().trim());
            if (!matchedClient && formData.clientName.trim() !== '') {
                try {
                    const newClient = await ClientsService.create({ name: formData.clientName.trim() });
                    finalClientId = newClient.id;
                } catch (err) {
                    console.error("Error creating client during edit:", err);
                            ...
                            <div key={index} className="space-y-6 p-6 rounded-2xl bg-muted/30 border border-border relative group/item transition-colors shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input value={item.metal} onChange={(e) => handleItemChange(index, 'metal', e.target.value)} className="w-full bg-muted border border-border rounded-xl py-3 px-4 text-sm text-foreground focus:border-indigo-500/30 outline-none transition-colors shadow-inner" placeholder="Oro, Plata" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-muted-foreground text-[9px] font-black uppercase tracking-widest px-1 transition-colors">Color</label>
                                        <input value={item.color} onChange={(e) => handleItemChange(index, 'color', e.target.value)} className="w-full bg-muted border border-border rounded-xl py-3 px-4 text-sm text-foreground focus:border-indigo-500/30 outline-none transition-colors shadow-inner" placeholder="Blanco..." />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <input value={item.karats} onChange={(e) => handleItemChange(index, 'karats', e.target.value)} className="w-full bg-muted border border-border rounded-xl py-3 px-4 text-sm text-foreground focus:border-indigo-500/30 outline-none transition-colors shadow-inner" placeholder="14k" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-muted-foreground text-[9px] font-black uppercase tracking-widest px-1 transition-colors">Peso (Gr)</label>
                                        <input value={item.weight} onChange={(e) => handleItemChange(index, 'weight', e.target.value)} className="w-full bg-muted border border-border rounded-xl py-3 px-4 text-sm text-foreground focus:border-indigo-500/30 outline-none transition-colors shadow-inner" placeholder="0.0" />
                                    </div>
                                    <div className="space-y-2">
